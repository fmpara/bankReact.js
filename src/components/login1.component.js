...
const LoginTest = () => {
  ...
  const Login = async () => {
    ...
    async function data() {
      ...
      let formField = new FormData();
      formField.append("token", localStorage["token"]);
      await axios({
        method: "post",
        ...
        .then((res) => {
          ...
          localStorage.setItem("Accounts", zmienna);
          localStorage.setItem("Waluty", zmiennaWaluty);
    ...
    async function check() {
      if (sessionStorage["loggedIn"] === "1") history.push("./user1");
    }
  };
