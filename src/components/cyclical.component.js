...
const Withdraw = () => {
  ...
  async function przelew() {
    ...
    if (waluts.includes("USD")) {
      ...
      formField.append("date",withdrawDate)
      console.log(numberBankNadawcyUSD[0]);

      await axios({
        method: "post",
        url: "https://lorekdev.pl/api/cyklicznyPrzelew/",
        data: formField,
      }).then((res) => {
        setIsValid(true);
        localStorage.setItem("loggedIn", "1");
    ...
    return (
      <div className="auth-wrapper">
        ...
        <div className="auth-przelew">
          {isValid ? (
            <Alert variant="success">Udało się ustawić przelew</Alert>
          ) : null}

          <h3>Przelew cykliczny</h3>
          ...
          <div className="form-group">
            <label>Co ile dni wykonywać przelew</label>
            <input
              type="number"
              className="form-control"
              placeholder="dni"
